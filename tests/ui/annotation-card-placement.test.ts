...
// pure placement helper — CARD_WIDTH=288, PIN_RADIUS=16, CARD_GAP=12,
// CARD_GUTTER=8 are baked into these expected numbers; see the constants
// next to the function in annotation-layer.tsx if they ever change.
describe("placeCardLeft", () => {
  ...
  it("places the card to the left of a pin in the right half of the stage", () => {
    ...
    // pin: 900 - 16 - 12 - 288 (CARD_WIDTH) = 584.
    expect(placeCardLeft(900, 1000)).toBe(584);
  });
  ...
  it("falls back within the stage bounds when the preferred side would overflow", () => {
    ...
    // side is right — but 100 + 16 + 12 + 288 = 416 runs well past the
    ...
    expect(result).toBe(350 - 288 - 8); // 54 — clamped to the right-hand gutter
    expect(result + 288).toBeLessThanOrEqual(350 - 8);
  });

  it("clamps into the stage bounds when the container is narrower than the card itself", () => {
    // A 200px-wide stage can't fit a 288px card with gutters on either side
  ...
  describe("with an explicit cardWidth", () => {
    it("matches the default-width result when passed the same value explicitly", () => {
      expect(placeCardLeft(100, 1000, 288)).toBe(placeCardLeft(100, 1000));
    });
